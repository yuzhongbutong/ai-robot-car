...
class DatabaseService:
    ...
    def query_settings(self, db):
        ...
        for _, item in enumerate(settings):
            ...
            result['data'][item[0]] = json.loads(text)
        return result
    ...
    def write_settings(self, db, source):
        ...
        for _, item in enumerate(config.SETTINGS_ITEMS):
            if item in source:
                text = json.dumps(source[item])
                text = crypto.encrypt(text)
                sql = 'REPLACE INTO TBL_SETTINGS VALUES (\'' + \
                    item + '\', \'' + text + '\')'
                cursor.execute(sql)
    ...
    def connect_settings(self, source):
        ...
        if config.SETTINGS_ITEMS_INTERNAL in keys:
            ...
            port = int(
                source[config.SETTINGS_ITEMS_INTERNAL].get('port', '1883'))
            ...
            userName = 'a-' + orgId + '-' + \
                source[config.SETTINGS_ITEMS_WATSON].get('userName')
        ...
        broker_service = BrokerService()
        is_connected = broker_service.connect(
            host, port, userName, password, client_id, user_data)
        if is_connected:
